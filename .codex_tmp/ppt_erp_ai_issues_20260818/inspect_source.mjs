...
const inspectDir = path.join(workspace, "source-inspect");
...
const slideIndex = await presentation.inspect({ kind: "slide", include: "id,slide,title,textShapes", maxChars: 50000 });
...
for (let index = 0; index < presentation.slides.items.length; index += 1) {
  ...
  await writeBlob(path.join(inspectDir, `slide-${number}.png`), await presentation.export({ slide, format: "png", scale: 1 }));
...
await fs.writeFile(path.join(inspectDir, "source-inspect.ndjson"), `${combined.filter(Boolean).join("\n")}\n`, "utf8");
await fs.writeFile(path.join(workspace, "source-manifest.json"), JSON.stringify({
  source,
  slideCount: presentation.slides.items.length,
  slideSize: presentation.slideSize,
  masters: presentation.masters.items.map((master) => ({ id: master.id, name: master.name, placeholders: master.placeholders?.summary?.() ?? [] })),
  layouts: presentation.layouts.items.map((layout) => ({ id: layout.id, name: layout.name, parentLayoutId: layout.parentLayoutId, placeholders: layout.placeholders?.summary?.() ?? [] })),
}, null, 2), "utf8");
