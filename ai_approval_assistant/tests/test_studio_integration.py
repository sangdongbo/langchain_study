...
def test_langgraph_json_points_to_importable_compiled_graph() -> None:
    ...
    assert "daily_report_form_agent" in graph.get_graph().nodes
    ...
    assert ("daily_report_form_agent", "__end__") in edges
...
def test_intent_router_routes_daily_report_requests_to_form_agent() -> None:
    ...
    assert result["_route"] == "daily_report_form_agent"


def test_intent_router_routes_fast_daily_report_requests_to_chat_agent() -> None:
    module = importlib.import_module("app.agents.approval_agent")
    state = module.initial_state("S-fast-daily-router", "863")
    state["user_message"] = "快速写日报：今天完成客户跟进"
...
def test_intent_router_keeps_daily_report_confirmation_on_form_agent() -> None:
    module = importlib.import_module("app.agents.approval_agent")
