...
1. 逐页读取 PDF。
2. 检查空白页，避免扫描页被静默忽略。
3. 按字符窗口切分 Chunk，保留页码、原文件名、版本、生效日期和权限标签。
...
- `ERP_MODE=remote`：需要请求头 `UID`、`Authorization`，调用 `/api/User/userinfo`、`/api/approval/list`、`/api/field/formFields`、可配置的审批状态接口、`/api/approval/getNodes` 和 `/api/approval/add`。
- `ERP_MODE=mock`：仅用于离线排练，所有返回都会标记 `erp_mode=mock`，不会伪装成真实 ERP 写入。
