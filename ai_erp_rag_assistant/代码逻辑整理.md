...
| `app/api/router.py` | `/api` 根路由注册，不承载身份或业务逻辑 |
| `app/api/dependencies.py` | ERP 身份校验、租户隔离和 RAG 运行配置依赖 |
| `app/api/schemas.py` | 聊天、审批、会话、RAG 和文档接口契约 |
| `app/api/admin_schemas.py` | RAG 后台管理接口契约 |
| `app/api/routes/chat.py` | 统一聊天入口、身份上下文、工作流选择和 SSE 传输 |
| `app/api/routes/rag.py` | 直接检索、RAG 问答、同步文档导入和失败任务重试 |
| `app/api/routes/rag_documents.py` | 文档列表、检索开关和删除 |
| `app/api/routes/assistants.py` | 前端助手目录 |
| `app/api/routes/approvals.py` | 审批模板、表单和候选项接口 |
| `app/api/routes/sessions.py` | 长期会话列表、消息、重命名和逻辑删除 |
| `app/api/routes/executions.py` | Durable Execution 状态查询 |
| `app/api/routes/rag_admin.py` | Assistant、Prompt、知识库和数据源管理接口 |
| `app/repositories/rag_admin.py` | RAG 后台配置与运行时知识库范围查询 |
...
| `app/services/rag_ingest_service.py` | 同步导入流水线和任务阶段编排 |
| `app/services/chat_execution_service.py` | 响应转换、Durable 完成/失败和会话持久化编排 |
...
1. `chat_execution_service` 将内部状态转换为稳定 `ChatResponse`。
2. 同一服务统一处理 JSON/SSE 的 Durable 完成或失败状态。
3. Durable Run 先记录完成结果，长期会话再保存消息和脱敏状态。
