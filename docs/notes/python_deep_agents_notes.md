...
DATA_PATH = PROJECT_ROOT / "docs" / "examples" / "sample-data" / "procurement_business_data.json"
...
子 Agent、Backend、动态 Skills、Sandbox 和 Harness 属于进阶能力，已拆分到独立文档：[Deep Agents 进阶能力专题](../guides/deep_agents_capabilities.md)。

完成下面的报告生成与程序化验收后，再阅读该专题会更连贯。


## 12. 虚拟文件系统：让 Agent 写一份报告
...
完整路线与毕业项目见：[Deep Agents 进阶学习路线：从会调用工具到可上线](../guides/deep_agents_advanced_learning.md)。

更系统的工程思想见：[Harness Engineering：从模型能力到可靠 Agent 系统](../guides/harness_engineering.md)。

准备继续研究框架内部机制时，阅读：[Deep Agents 源码研究与实验手册](../guides/deep_agents_source_research.md)。
