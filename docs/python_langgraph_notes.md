...
# ChatDeepSeek 是 DeepSeek 在 LangChain 里的聊天模型封装。
...
model = ChatDeepSeek(
    # DeepSeek 的普通聊天模型，适合大多数 Agent 示例。
    model="deepseek-chat",
    # 从环境变量读取密钥，避免把 key 写死到代码里。
...
2. `ChatDeepSeek(...)`：创建 DeepSeek 模型对象，后续 Agent 的理解、决策和回复都由它完成。
...
model = ChatDeepSeek(
    # 模型名称。学习阶段一般用 deepseek-chat。
    model="deepseek-chat",
    # 环境变量里需要提前配置 DEEPSEEK_API_KEY。
    api_key=os.getenv("DEEPSEEK_API_KEY"),
    # 低随机性更适合工具调用和文档示例。
    temperature=0,
)
...
model = ChatOpenAI(
    # DeepSeek 的 OpenAI 兼容接口也使用 deepseek-chat 作为模型名。
    model="deepseek-chat",
    api_key=os.getenv("DEEPSEEK_API_KEY"),
    # 关键点：把 base_url 指向 DeepSeek，而不是默认 OpenAI 地址。
...
agent.invoke(
    ...
    # config 适合传“本次运行固定不变”的配置。
    # configurable 里的内容不会直接暴露给模型作为工具参数。
