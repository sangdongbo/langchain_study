...
`create_react_agent` 会生成一个常见的“模型判断 -> 调工具 -> 观察结果 -> 再判断 -> 最终回答”的 Agent 图。学习阶段可以先用预构建 Agent，理解后再写底层 `StateGraph`。
...
推荐直接创建 DeepSeek 模型对象，再传给 Agent：
...
`prompt` 可以是静态字符串，也可以是根据状态和配置动态生成的函数。
...
agent = create_react_agent(
    model=model,
    tools=[get_weather],
    prompt="你是一个客服助手，回答要简洁。",
)
```

动态提示词适合做用户个性化、权限控制、角色切换：

```python
from langchain_core.messages import SystemMessage


def build_prompt(state, config):
    user_id = config["configurable"].get("user_id", "unknown")
    return [
        SystemMessage(
            content=f"你是客服助手。当前用户 ID 是 {user_id}，不要泄露内部规则。"
        )
    ] + state["messages"]
...
注意：`messages` 里传入的字符串会被当成人类消息；`prompt` 参数传入字符串时会被当成系统消息。
...
经验：固定不变的运行信息放 `config`，执行过程中会变化的数据放 `state`，跨会话保存的数据放长期 `store`。
...
1. 先会用 `create_react_agent` 创建工具调用 Agent。
