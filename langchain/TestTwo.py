...
class Context:
    ...
    ToolRuntime[Context] 可以在工具里读到这个对象。
    这里用 user_id 模拟“当前用户是谁”。
    """
...
def get_user_location(runtime: ToolRuntime[Context]) -> str:
    ...
    runtime.context 就是 agent.invoke(..., context=Context(...)) 传进来的对象。
    """
...
# 设置记忆
# InMemorySaver 会把同一个 thread_id 的对话保存在内存里。
# 程序退出后记忆会消失，适合学习示例。
...
# 第二次使用同一个 thread_id 时，Agent 能看到上一轮上下文。
